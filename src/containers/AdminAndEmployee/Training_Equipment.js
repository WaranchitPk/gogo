...
class Training_Equipment extends Component {
    ...
    componentDidMount() {
        ...
        axios.get('http://localhost:9999/api/training_equipment/home/findStatus').then(result => {
            this.setState({
                status: result
            })
        })
    ...
    render() {
        ...
        if (this.state.status !== null && this.state.status !== undefined) {
            // resultStatusMachine = this.state.status.data.data.result
            resultStatusMachine = this.state.status.data
        ...
        return (
            <div>
                <TrainingEquipmentComponent
                    ...
                    resultStatus={resultStatusMachine}/>
            </div>
        );
    }
}
